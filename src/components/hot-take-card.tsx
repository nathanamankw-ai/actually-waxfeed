...
function TemperatureGauge({ agreeCount, disagreeCount }: { agreeCount: number; disagreeCount: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 h-1 bg-[#222] relative overflow-hidden">
        {/* Agree side (left) */}
        <div
      ...
      <span
        ...
        style={{ color: heat > 50 ? "#ff3b3b" : "#666" }}
      >
  ...
  if (compact) {
    return (
      <Link
        ...
        className="block group border-b border-[#222] py-6 hover:bg-[#111] transition-colors -mx-4 px-4"
      >
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Album info */}
            <p className="text-sm font-bold truncate mt-1 group-hover:text-[#888] transition-colors">
              {hotTake.albumTitle}
            </p>
            <p className="text-xs text-[#666] truncate">{hotTake.albumArtist}</p>

            {/* Vote counts */}
            <div className="flex gap-4 mt-2 text-[10px] tracking-[0.1em] uppercase text-[#666]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4 aspect-square lg:aspect-auto">
          <Link href={`/album/${hotTake.albumSpotifyId}`} className="block h-full">
            <div className="relative h-full min-h-[280px] lg:min-h-full bg-[#111] overflow-hidden group">
              {hotTake.albumCoverUrl && (
                <img
                  src={hotTake.albumCoverUrl}
                  alt={hotTake.albumTitle}
        ...
        <div className="lg:col-span-8 p-6 flex flex-col">
          {/* The hot take itself */}
          <div className="mb-6">
            <p className="text-[10px] tracking-[0.2em] uppercase text-[#666] mb-2">
              The Take
            </p>
              ...
              <div className="space-y-4">
                {hotTake.topAgreeArgument && (
                  <div className="pl-4 border-l-2 border-white">
                    <p className="text-sm text-[#ccc]">{hotTake.topAgreeArgument.content}</p>
                ...
                {hotTake.topDisagreeArgument && (
                  <div className="pl-4 border-l-2 border-[#ff3b3b]">
                    <p className="text-sm text-[#ccc]">{hotTake.topDisagreeArgument.content}</p>
                    <p className="text-[10px] tracking-[0.1em] uppercase text-[#666] mt-2">
          ...
          <Link
            href={`/hot-takes/${hotTake.id}`}
            className="mt-6 text-[10px] tracking-[0.2em] uppercase text-[#666] hover:text-white transition-colors"
          >
            View Full Debate →
          </Link>
        </div>
