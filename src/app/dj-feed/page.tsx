...
export default async function DJFeedPage() {
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 lg:py-8">
      {/* Hero Section */}
      <div className="relative bg-gradient-to-br from-cyan-900/40 via-teal-900/30 to-emerald-900/20 rounded-2xl p-8 mb-8 overflow-hidden">
        <div className="absolute inset-0 bg-[url('/noise.png')] opacity-5" />
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-4xl">🎧</span>
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold">DJFeed</h1>
              <p className="text-cyan-200/80">Discover DJs, book talent, share mixes</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href="/dj-feed/apply"
              className="flex items-center gap-2 bg-cyan-500 text-black px-6 py-3 font-bold rounded-lg hover:bg-cyan-400 transition-colors no-underline"
            >
            ...
            <Link
              href="/live"
              className="flex items-center gap-2 border-2 border-cyan-500/50 text-cyan-200 px-6 py-3 font-bold rounded-lg hover:bg-cyan-500/10 transition-colors no-underline"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dj-feed/all" className="text-sm text-[#888] hover:text-white no-underline">
                View All →
              </Link>
            </div>
              ...
              {FEATURED_DJS.map((dj) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    {dj.image ? (
                      <img src={dj.image} alt="" className="w-16 h-16 rounded-full" />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[#888] line-clamp-2 mt-1">{dj.bio}</p>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {dj.genres.slice(0, 3).map((genre) => (
                          <span key={genre} className="text-xs bg-[#222] text-[#888] px-2 py-0.5 rounded">
                            {genre}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-[#222]">
                    <span className={`text-sm ${dj.isAvailable ? "text-green-500" : "text-[#666]"}`}>
                      {dj.isAvailable ? "✓ Available for booking" : "Currently unavailable"}
                    </span>
                    <span className="text-sm text-[#888]">${dj.hourlyRate}/hr</span>
                  </div>
                </Link>
              ))}
            </div>
              ...
              {RECENT_MIXES.map((mix) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 bg-gradient-to-br from-cyan-500/30 to-teal-500/30 rounded-lg flex items-center justify-center">
          ...
          {/* Book a DJ CTA */}
          <section className="bg-gradient-to-r from-cyan-500/20 to-teal-500/20 border border-cyan-500/30 rounded-lg p-6">
            ...
            <Link
              href="/dj-feed/book"
              className="inline-block bg-cyan-500 text-black px-6 py-3 font-bold rounded-lg hover:bg-cyan-400 transition-colors no-underline"
            >
              Browse & Book DJs
            </Link>
          </section>
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {["Hip-Hop", "House", "Techno", "R&B", "Electronic", "Soul", "Funk", "Ambient"].map((genre) => (
                <Link
                  ...
                  className="text-sm bg-[#222] hover:bg-[#333] px-3 py-1.5 rounded-full transition-colors no-underline"
                >
                  {genre}
                </Link>
              ))}
          ...
          <div className="bg-[#111] border border-[#222] rounded-lg p-4">
            ...
            <Link
              href="/dj-feed/apply"
              className="block w-full text-center bg-cyan-500 text-black py-2 font-bold rounded hover:bg-cyan-400 transition-colors no-underline"
            >
              Apply Now
            </Link>
          </div>
