...
export function ConversationChat({
  ...
  const [isTyping, setIsTyping] = useState<string[]>([]) // usernames who are typing
  ...
  useEffect(() => {
    const pollMessages = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.typing) {
            setIsTyping(data.typing.filter((u: string) => u !== currentUserId))
          }
        }
      } catch (e) {
    ...
    pollMessages()
    const interval = setInterval(pollMessages, 2000) // Faster polling for better real-time feel
  ...
  const handleEditMessage = async (messageId: string) => {
    ...
    try {
      const res = await fetch(`/api/messages/${conversationId}/messages/${messageId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex flex-col h-full bg-white">
                          ...
                          {message.type === "album" && message.metadata?.albumId ? (
                            ...
                            <div className="max-w-2xl">
                              <textarea
                                ...
                                className="w-full bg-gray-50 border border-gray-300 p-2 text-sm resize-none focus:outline-none focus:border-black"
                                rows={2}
                                autoFocus
                              />
      ...
      {/* Typing Indicator */}
      {isTyping.length > 0 && (
        <div className="px-6 py-2 text-xs text-gray-500 flex items-center gap-2">
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2">
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder={replyingTo ? `Reply to ${replyingTo.user.username}...` : "Type a message... (@ to mention)"}
              disabled={isLoading}
              rows={1}
